...
Skill files are siblings. Resolve `ruver-qa` as the folder next to
`ruver-bus`, or via the host skill loader. Never hardcode
`~/.agents/skills`, `~/.grok`, `~/.claude`, or `~/.codex`.

## Primitives

| Primitive | Meaning | Fallback if the host has no API |
|---|---|---|
| `load_skill name` | Inject that skill's SKILL.md (and GRAPH.md if present) into **this** thread | Read the sibling `skills/<name>/SKILL.md` |
...
`$RUVER_ROOT` is host-neutral. See [skills/ruver-bus/DISK.md](skills/ruver-bus/DISK.md).
